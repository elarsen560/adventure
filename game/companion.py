...
def build_companion_prompt(context: str, question: str | None) -> str:
    ...
    return (
        ...
        "Your reply should primarily do one thing: recap, connect, surface tension, or focus attention.\n"
        "Answer from the strongest incomplete thread only.\n"
        ...
        "Include one concrete thread the player could follow next, derived only from known information, but do not enumerate options.\n"
        ...
        "Do not talk about commands, parser syntax, accepted words, game logic, or author intent.\n"
        "If the difficulty seems to be phrasing or approach, describe it as uncertainty in how the mechanism is being approached in-world, not as a command problem.\n"
        "If the player is probing something that is not currently actionable, acknowledge it as context or atmosphere without implying hidden mechanics.\n"
        ...
        "If evidence is thin, say so briefly and keep attention on the strongest visible object, place, or mechanism.\n\n"
        "Context:\n"
        f"{context}\n"
        "Player request:\n"
        f"{player_request}"
